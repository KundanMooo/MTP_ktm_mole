...
def load_molecules(path, label):
    suppl = Chem.SDMolSupplier(path)
    return [(m, label) for m in suppl if m]

# Main processing
def main(args):
    # load
    active = load_molecules(args.active, 1)
    inactive = load_molecules(args.inactive, 0)
    # balance
    n = min(len(active), len(inactive))
    data = active[:n] + inactive[:n]
    np.random.shuffle(data)
    # compile SMARTS
    groups = get_functional_groups()
    patterns = {name: Chem.MolFromSmarts(s) for name, s in groups.items()}
    # build rows
    rows = []
    for idx, (mol, lbl) in enumerate(data):
        row = {'mol_id': idx}
        for name, patt in patterns.items():
            row[name] = int(mol.HasSubstructMatch(patt))
        row['target'] = lbl
        rows.append(row)
    # DataFrame
...
if __name__ == '__main__':
    ...
    p.add_argument('--active',   required=True, help='Path to active.sdf')
    p.add_argument('--inactive', required=True, help='Path to inactive.sdf')
